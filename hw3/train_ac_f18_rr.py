...
def build_mlp(input_placeholder, output_size, scope, n_layers, size, activation=tf.tanh, output_activation=None):
    ...
    hidden_output = tf.contrib.layers.repeat(
        input_placeholder, n_layers-1, tf.contrib.layers.fully_connected, size, 
...
class Agent(object):
    ...
    def policy_forward_pass(self, sy_ob_no):
        ...
        """
        net_out = build_mlp(sy_ob_no, self.ac_dim, 'policy', self.n_layers, self.size)
        if self.discrete:
            sy_logits_na = net_out
            return sy_logits_na
        else:
            sy_mean = net_out
            sy_logstd = tf.Variable(np.random.random(self.ac_dim), expected_shape=[self.ac_dim], trainable=True, dtype=tf.float32)
    ...
    def sample_action(self, policy_parameters):
        ...
        if self.discrete:
            ...
            sy_sampled_ac = sy_mean + sy_std * z_sample_na
    ...
    def get_log_prob(self, policy_parameters, sy_ac_na):
        ...
        """
        if self.discrete:
            # log prob under categorical distribution
            sy_logits_na = policy_parameters
            ...
            # log prob under multivariate gaussian
            sy_mean, sy_logstd = policy_parameters
            sy_std = tf.exp(sy_logstd)
    ...
    def build_computation_graph(self):
        ...
        # policy takes in an observation and produces a distribution over the action space
        self.policy_parameters = self.policy_forward_pass(self.sy_ob_no)

        # actions sampled from distribution
        self.sy_sampled_ac = self.sample_action(self.policy_parameters)

        # logprob of the actions that were actually taken by the policy
    ...
    def sample_trajectory(self, env, animate_this_episode):
        ...
        while True:
            ...
            # get action from policy, take the action, store results 
            obs.append(ob)
            o = np.expand_dims(ob, axis=0)
            ac = self.sess.run(
                self.sy_sampled_ac,
                feed_dict={self.sy_ob_no : o} )
            ac = ac[0]
            acs.append(ac)
    ...
    def estimate_advantage(self, ob_no, next_ob_no, re_n, terminal_n):
        ...
        arguments:
            ...
            next_ob_no: shape: (sum_of_path_lengths, ob_dim). The observation after taking one step 
                forward
            re_n: length: sum_of_path_lengths. Each element in re_n is a scalar containing the reward 
                for each timestep
            terminal_n: length: sum_of_path_lengths. Each element in terminal_n is either 1 if the 
                episode ended at that timestep of 0 if the episode did not end

        returns:
            adv_n: shape: (sum_of_path_lengths). A single vector for the estimated advantages whose 
                length is the sum of the lengths of the paths
        """
        # critic estimation of current state value
        V_t_n = self.sess.run(
            self.critic_prediction,
            feed_dict={self.sy_ob_no : ob_no})

        # critic estimation of next state value
        V_tp1_n = self.sess.run(
            self.critic_prediction,
            feed_dict={self.sy_ob_no : next_ob_no})

        Q_t_n = re_n + self.gamma * np.multiply(V_tp1_n, terminal_n)
    ...
    def update_critic(self, ob_no, next_ob_no, re_n, terminal_n):
        """
        Update the parameters of the critic. This must be done after every policy update.
        ...
        returns:
            nothing
        """
        for i in range(0, self.num_target_updates):
            V_tp1_n = self.sess.run(
                self.critic_prediction,
                feed_dict={self.sy_ob_no : next_ob_no})
            # don't need to calculat V_t_n, because this is calculated by critic update op

            target = re_n + self.gamma * np.multiply(V_tp1_n, terminal_n)

            for j in range(0, self.num_grad_steps_per_target_update):
                self.sess.run(
                    self.critic_update_op,
                    feed_dict={
                        self.sy_target_n : target,
                        self.sy_ob_no : ob_no})
        return
...
def train_AC(
    ...
    for itr in range(n_iter):
        ...
        # update critic with data batch
        agent.update_critic(ob_no, next_ob_no, re_n, terminal_n)
        # use updated critic to estimate advantages
