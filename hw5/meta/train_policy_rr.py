...
from point_mass_rr import PointEnv 
from point_mass_observed_rr import ObservedPointEnv
...
def build_mlp(x, output_size, scope, n_layers, size, activation=tf.tanh, output_activation=None, 
    ...
    for i in range(n_layers):
        x = tf.layers.dense(
            inputs=x,units=size, activation=activation, name='fc{}'.format(i), 
...
def build_rnn(x, h, output_size, scope, n_layers, size, activation=tf.tanh, 
    ...
    """
    builds a gated recurrent neural network. inputs are embedded by a MLP and then passed to a
    GRU cell

    size = hidden layer size in MLP
    output_size = number of units in gru cell

    arguments:
        see build_policy()
    
    returns:
        x: output placeholder of network
        h: hidden state of GRU cell
    """
    # embed inputs with a MLP
    embedded_inputs = build_mlp(
        x, size, scope, n_layers, size, activation=activation, output_activation=output_activation)

    # embedded inputs are passed to GRU cell
    gru_cell = tf.nn.rnn_cell.GRUCell(
        output_size, activation=activation)

    # rnn
    outputs, last_states = tf.nn.dynamic_rnn(
        cell=gru_cell, inputs=embedded_inputs, initial_state=h)

    return last_states, outputs
...
def build_policy(x, h, output_size, scope, n_layers, size, gru_size, recurrent=True,
    ...
    arguments:
        x: plaeholder variable for the input, which has dimension (batch_size, hisgory, input_size)
        ...
        recurrent: if the network should be recurrent of feedforward
    ...
    we predict the mean and std of the gaussian policy, and we don't want the std to start off too 
    large. thus initialize the last layer of the policy with a values ~ N(0, 0.01)
    """
...
class Agent(object):
    def __init__(self,
            computation_graph_args, sample_trajectory_args, estimate_return_args):
    ...
    def define_placeholders(self):
        """
        placeholders for batch batch observations / actions / advantages in policy gradient loss 
        function. see Agent.build_computation_graph for notation
    ...
    def policy_forward_pass(self, sy_ob_no, sy_hidden):
        """
        constructs the symbolic operation for the policy network outputs, which are the parameters 
        of the policy distribution p(a|s)
        ...
        returns:
            ...
            the parameters are a tuple (mean, log_std) of a Gaussian distribution over actions. 
            log_std should just be a trainable variable, not a network output.
        ...
        sy_policy_params, sy_hidden = build_policy(
            sy_ob_no, sy_hidden, self.ac_dim*2, self.scope, n_layers=self.n_layers, size=self.size, 
    ...
    def sample_action(self, policy_parameters):
        """
        constructs symbolic operation for stochastically sampling from the policy distribution
        ...
        returns:
            sy_sampled_ac: (batch_size, self.ac_dim)
        """
    ...
    def get_log_prob(self, policy_parameters, sy_ac_na):
        """
        constructs a symbolic operation for computing the log probability of a set of actions that 
        were action taken according to the policy
    ...
    def sample_trajectory(self, env, animate_this_episode, is_evaluation):
        ...
        max(self.hisgory, self.max_path_length) timesteps have been sampled

        construct meta-observations by concatenating (s, a, r, d) into one vector inputs to the policy
        should have the shape (batch_size, self.history, self.meta_ob_dim). zero pad the input to 
        maintain a consistent input shape

        add the entire input as observation to the replay buffer, along with a, r, d. samples will be
        drawn from the replay buffer to update the policy
        ...
        while True:
            ...
            if ep_steps == 0:
                ...
                meta_obs[(steps + self.history), 0:self.ob_dim] = ob
                steps += 1
    ...
    def compute_advantage(self, ob_no, re_n, hidden, masks, tau=0.95):
        ...
        arguments:
            ob_no: (batch_size, history, ob_dim)
            rewards: (batch_size, )
            masks: (batch_size, )
            values: (batch_size, )
            gamma: scalar
    ...
    def estimate_return(self, ob_no, re_n, hidden, masks):
        """
        estimates returns over a set of trajectories

        arguments:
            ob_no: (sum of path lengths, history, meta_obs_dim)
            re_n: (sum of path_lengths, ), each element is a sum of rewards over a trajectory
            hidden: hidden state of recurrent policy
            masks: terminals masks

        returns:
            q_n: (sum of path lengths, ), estimated q values
            adv_n: (sum of path lengths, ), estimated advantages
        """
    ...
    def update_parameters(self, ob_no, hidden, ac_na, fixed_log_probs, q_n, adv_n):
        """
        update parameters of the policy and critic with PPO update

        arguments:
            ob_no: (minibatch_size, history, meta_obs_dim)
            hidden: (minibatch_size, self.gru_size)
            ac_na: (minibatch_size, )
            fixed_log_probs: (minibatch_size, )
            adv_n: (minibatch_size, )
            q_n: (sum of path lengths, )
        """
